...
std::shared_ptr<Texture2D> Texture2D::create(const std::string& name, int width, int height, unsigned char* data, const TextureProps& props)
{
	switch (RendererAPI::getAPI())
	{
	case RendererAPI::API::None:
		...
		return nullptr;
	case RendererAPI::API::OpenGL:
		return std::make_shared<GLTexture2D>(name, width, height, data, props);
	}
}
