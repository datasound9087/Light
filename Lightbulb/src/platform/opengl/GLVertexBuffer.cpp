...
GLVertexBuffer::GLVertexBuffer(size_t size, void* data, const Usage& usage)
{
	this->type = DataTypes::Types::None;
	this->size = size;
	this->usage = usage;
	...
	glGenBuffers(1, &id);
	bind();
	glBufferData(GL_ARRAY_BUFFER, size, data, usage == VertexBuffer::Usage::STATIC ? GL_STATIC_DRAW : GL_DYNAMIC_DRAW);
}
